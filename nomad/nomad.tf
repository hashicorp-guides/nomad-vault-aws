...
data "terraform_remote_state" "admin" {
  backend = "local"
  config {
      path = "${path.module}/../admin/admin.tfstate"
  }
}
...
provider "consul" {
  address    = "${data.terraform_remote_state.admin.admin_ip}:8500"
  datacenter = "dc1"
}
data "consul_keys" "nomad_vault" {
  key {
    name    = "token"
    path    = "service/vault/root-token"
    default = ""
  }
}
...
data "template_file" "init" {
  ...
  vars = {
    vault_token      = "${data.consul_keys.nomad_vault.var.token}"
...
resource "aws_autoscaling_group" "nomad_server" {
  ...
  vpc_zone_identifier  = ["${data.terraform_remote_state.vault.subnet_public_ids}"]
  ...
  tag {
    ...
    value               = "${data.terraform_remote_state.vault.environment_name}"
    propagate_at_launch = true
  }
}
